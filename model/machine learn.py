...
FEATURES= ["COM_RAT", "Cyclic", "Dcy*", "DPT*", "LCOM", "Level", "INNER", "jf", "Level*", "String processing", "PDpt", "CLOC", "JLOC", "Jm","Business Logic"]
...
def main():
    try:
        ...
        print(f"Training set cleaning completed. Original size: {original_train_size}, Processed size: {len(df_train_clean)}")
        print("Training set target column distribution:\n", df_train_clean["1-suit-LLM"].value_counts())
        print(f"Testing set cleaning completed. Original size: {original_test_size}, Processed size: {len(df_test_clean)}")
        print("Testing set target column distribution:\n", df_test_clean["1-suit-LLM"].value_counts())
